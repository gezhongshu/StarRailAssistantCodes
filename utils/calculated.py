...
class calculated:
    ...
    def click_target(self, target_path: str, threshold, flag:bool=True, check:bool=False, map=""):
        ...
        if temp_name not in temp_ocr or join:
            ...
            start_time = time.time()
            first_timeout = True
            distance_iter_x = itertools.cycle([800, -500, -500, 0, 0, 500, 500, 0])
            distance_iter_y = itertools.cycle([800, 0, 0, -500, -500, 0, 0, 500])
            ...
            while True:
                ...
                if time.time() - start_time > 5 and "point" in temp_name:
                    ...
                    pyautogui.mouseDown()
                    pyautogui.moveTo(start_x+next(distance_iter_x), start_y+next(distance_iter_y), duration=1)
                    pyautogui.mouseUp()
                    move_num+=1
    ...
    def wait_join(self):
        ...
        block_join1 = False
        block_join2 = False
        pc_join = self.data.get("join_time", 2)
        while True:
